    ...
    public class BackingStore
    {
        ...
        public bool SaveProfileOld(int device, string proName)
        {
            ...
            try
            {
                ...
                XmlNode xmlRSMergeY = m_Xdoc.CreateNode(XmlNodeType.Element, "RSMergeY", null); xmlRSMergeY.InnerText = rsMergeY[device].ToString(); rootElement.AppendChild(xmlRSMergeY);
                XmlNode xmlLSMergeY = m_Xdoc.CreateNode(XmlNodeType.Element, "LSMergeY", null); xmlLSMergeY.InnerText = lsMergeY[device].ToString(); rootElement.AppendChild(xmlLSMergeY);
            ...
            string propath = "", bool xinputChange = true, bool postLoad = true)
        {
            ...
            if (File.Exists(profilepath))
            {
                ...
                try
                {
                    Item = m_Xdoc.SelectSingleNode("/" + rootname + "/RSMergeY");
                    bool.TryParse(Item.InnerText, out bool temp);
                    rsMergeY[device] = temp;
                }
                catch { rsMergeY[device] = false; missingSetting = true; }

                try
                {
                    Item = m_Xdoc.SelectSingleNode("/" + rootname + "/LSMergeY");
                    bool.TryParse(Item.InnerText, out bool temp);
                    lsMergeY[device] = temp;
                }
                catch { lsMergeY[device] = false; missingSetting = true; }
        ...
        private void ResetProfile(int device)
        {
            ...
            rsMergeY[device] = false;
            lsMergeY[device] = false;
